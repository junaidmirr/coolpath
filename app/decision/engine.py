...
def _compute_routes_blocking(mission: Mission, offsets: List[int], provider, activity_cfg: dict):
    """
    Downloads base street network once for the given activity (walk, bike, drive)
    and computes candidate routes for all offsets. Runs inside ThreadPoolExecutor.
    """
    ...
    for offset in offsets:
        try:
            ...
            logger.warning(f"Routing failed for offset {offset} ({network_type}): {e}")
...
async def optimize_mission(mission: Mission, provider) -> Dict[str, Any]:
    ...
    if not feasible_actions:
        return {
            "decision": "HIGH HEAT — BEST AVAILABLE PLAN",
            "planning_mode": planning_mode,
            "wait_minutes": 0,
            "recommended_action": None,
            "comparison": None,
            ...
            "routes": None,
            "route_options": [],
            "details": "No feasible actions meet the deadline.",
            "explanation": f"Warning: No {mission.activity} option meets your deadline under current conditions."
        }
