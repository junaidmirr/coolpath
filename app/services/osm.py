...
def crop_subgraph_for_bounds(master_g: nx.MultiDiGraph, origin: Coordinate, destination: Coordinate, buffer: float = 0.005) -> nx.MultiDiGraph:
...
def build_corridor_fallback_graph(origin: Coordinate, destination: Coordinate, base_speed_mps: float = 1.4) -> nx.MultiDiGraph:
    """Builds a high-density natural street network corridor between origin and destination."""
    multi_g = nx.MultiDiGraph()
    multi_g.graph["crs"] = "epsg:4326"
    multi_g.graph["name"] = "corridor_fallback_grid"
    
    lat_min = min(origin.lat, destination.lat) - 0.002
    lat_max = max(origin.lat, destination.lat) + 0.002
    lng_min = min(origin.lng, destination.lng) - 0.002
    lng_max = max(origin.lng, destination.lng) + 0.002
...
def download_street_network(origin: Coordinate, destination: Coordinate, network_type: str = "walk"):
    ...
    if master_g is not None:
        try:
            ...
            if len(sub_g.nodes) >= 10 and len(sub_g.edges) >= 10:
    ...
    west = min(origin.lng, destination.lng) - 0.004
